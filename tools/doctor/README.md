...
| **codegen** | `cc -c` the emitted C (compile-only) | a **C build failure** — the analysis legs pass but the emitted C won't compile (a Class boxed as int, a reopened `Object` struct, an undeclared local). Classified `incompatible-type` / `unknown-type` / `redefinition` / `undeclared-identifier` / `arg-count-mismatch` with the offending symbol — and the **Ruby source line** when the build carries `#line` directives ([matz/spinel#1338](https://github.com/matz/spinel/issues/1338)). The bulk of real harness finds ([spinel-dev#10](https://github.com/OriPekelman/spinel-dev/issues/10)) |
